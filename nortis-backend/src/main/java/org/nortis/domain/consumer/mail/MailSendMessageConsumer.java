...
public class MailSendMessageConsumer implements MessageConsumer {
	 ...
	 */
	@Override
	public void consume(Message message) throws ConsumeFailureException {
		...
		try {
			...
			throw new ConsumeFailureException("メールの送信に失敗しました", ex);
		}
	}
	
	private MimeMessage createMessage(MailConsumer mailConsumer, Message message) throws ConsumeFailureException {
		...
		try {
			...
			throw new ConsumeFailureException("メールのメッセージ構築に失敗しました", ex);	
		}
		return mimeMessage;
	}
